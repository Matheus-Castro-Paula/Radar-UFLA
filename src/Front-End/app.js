// Radar UFLA — front-end estático (Sprint 1)
// Os dados abaixo são um mock local, sem back-end ainda.
// A integração real com a API acontece a partir da Sprint 2.
...
const ITEMS = [
  {
    ...
    descricao: 'Encontrado caído perto da escada do Bloco 3. Tem uma pulseirinha de identificação junto.',
    local: 'Bloco 3',
  ...
  },
  {
    ...
    descricao: 'Estava em cima de uma mesa na ala de estudos silenciosos da Biblioteca Central.',
  ...
  },
  {
    ...
    descricao: 'Encontrada no chão perto do caixa do RU. Tem cartões dentro, sem dinheiro visível.',
    local: 'RU',
  ...
  },
  {
    ...
    descricao: 'Perdida ontem à tarde, saindo do RU em direção ao estacionamento.',
    local: 'RU',
  ...
  },
  {
    ...
    descricao: 'Perdido essa semana, provavelmente no estacionamento perto do bloco de exatas.',
  ...
  },
  {
    ...
    descricao: 'Perdida hoje de manhã. Documento — sem foto publicada, apenas esta descrição.',
    local: 'Não informado',
...
function initFeed() {
  ...
  if (!feedEl) return; // não estamos na página do feed

  const tabs = document.querySelectorAll('.tab');
  const searchInput = document.getElementById('search');
  const localInput = document.getElementById('filtro-local');
  const categoriaChecks = document.querySelectorAll('.filters input[type="checkbox"]');
  ...
  function render() {
    ...
    const filtrados = ITEMS.filter((item) => {
      ...
      if (categorias.length && !categorias.includes(item.categoria)) return false;
    ...
    document.getElementById('count-achados').textContent =
      ITEMS.filter((i) => i.feed === 'achados').length;
    document.getElementById('count-perdidos').textContent =
      ITEMS.filter((i) => i.feed === 'perdidos').length;
  }
...
function initPublishPage() {
  ...
  if (!form) return; // não estamos na página de publicação
  ...
  form.addEventListener('submit', (event) => {
    ...
    // Sem back-end ainda (Sprint 1): apenas confirma visualmente o preenchimento.
    // A integração real com a API acontece na Sprint 2.
    alert('Formulário preenchido corretamente! O envio real será integrado na Sprint 2, quando o back-end estiver pronto.');
  });
}
